...
contextBridge.exposeInMainWorld("sincgames", {
  ...
  fetchTorrentRelease: (payload) => ipcRenderer.invoke("torrent:fetch-release", payload),
  updateTorrentReleaseSourcePassword: (payload) => ipcRenderer.invoke("torrent:update-source-password", payload),
  ...
  removeGame: (payload) => ipcRenderer.invoke("game:remove", payload),
